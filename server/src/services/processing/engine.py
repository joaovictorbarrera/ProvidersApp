...
import os
import numpy as np
from src.services.processing.graphs import get_graphs

NOT_STARTED_FILTER = ['provider-name', 'department', 'date-signed', 'duration', 'date-end-validity',
                                    'date-start-planning', 'days-reserved-planning', 'date-recommended-send', 'lights', 'days-remaining', 'current-status']
STARTED_FILTER = ['provider-name', 'department', 'date-signed', 'duration', 'date-end-validity',
                                    ...
                                    'date-prices-offer', 'date-prices-map', 'lights', 'days-remaining', 'current-status']
...
def handle_csv_file(file_path, excel=False):
    dfDisplay = process_csv_File(file_path) 
    
    if excel:
        return saveExcelFile(dfDisplay, file_path)
    
    return build_display_package(dfDisplay)   

def build_display_package(dfDisplay):

    dfNotStarted, dfStarted, dfSent, dfConcluded = get_tables(dfDisplay)
    
    dfNotStartedLights = dfNotStarted["lights"].tolist()
    dfStartedLights = dfStarted["lights"].tolist()
    dfNotStarted.drop('lights', axis=1, inplace=True) 
    dfStarted.drop('lights', axis=1, inplace=True) 
    ...
    return {
        "tables": [
        {
            "tableContent":dfNotStarted.to_html(classes="table_style", index=False),
            "name":"not-started",
            "lights": dfNotStartedLights
        },
        {
            "tableContent":dfStarted.to_html(classes="table_style", index=False),
            "name":"started",
            "lights": dfStartedLights
        },
        {
            "tableContent":dfSent.to_html(classes="table_style", index=False),
            "name":"sent",
            "lights":[]
        },
        {
            "tableContent":dfConcluded.to_html(classes="table_style", index=False),
            "name":"concluded",
            "lights":[]
        }],
        "graphs": [
...
def saveExcelFile(dfDisplay, file_path):
    excel_file_name = (os.path.basename(file_path).split(".")[0] + "_excel.xlsx")
    excel_fullpath = os.path.join(os.path.dirname(file_path), excel_file_name)
...
def process_csv_File(file_path):
    ...
    df['days-remaining'] = pd.Series(dtype="object")
    for days_total, days_remaining, index in zip(df["days-reserved-planning"], df['days-remaining-to-duedate-send'], range(0, df.shape[0])):
        df.at[index, 'lights'] = calcLights(days_total, days_remaining)
        df.at[index, 'days-remaining'] = calcTime(days_remaining)
